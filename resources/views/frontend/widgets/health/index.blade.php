...
<div class="HadithBox">
    ...
    @php
        ...
        //     ->get();
            ...
            $healths = App\Models\WidgetPost::where('widget_categorie_id', '=', 1)->latest()->take(2)->get();

    @endphp
    @foreach ($healths as $health)
    <div>
        <h3 style="font-size: 20px">{{$health->title}}</h3>
        {{-- <p class="adage-description">{{ $adage->description }}</p> --}}
        <p><a href="#" style="font-size: 0.9rem" class="adage-disabled">{{ $health->description }}</a></p>
        <p><a href="#" style="font-size: 0.9rem" class="adage-disabled">{{ $health->author }}</a></p>
    </div>
    @endforeach
</div>
